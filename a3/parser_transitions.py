...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        ###         3. Right Arc
        if transition == 'S':
            current_word = self.buffer.pop(0)
            self.stack.append(current_word)
        elif transition == 'LA':
            current_word = self.stack.pop(-2)
            self.dependencies.append((self.stack[-1], current_word))
        elif transition == 'RA':
            current_word = self.stack.pop(-1)
            self.dependencies.append((self.stack[-1], current_word))
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        for pp, transition in zip(current_batch, transitions):
